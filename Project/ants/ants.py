...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        dis = 0
        while now != beehive:
            if len(now.bees) != 0 and self.min_range <= dis <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        aoe(self, damage = (amount >= self.armor) * self.damage + amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.digesting == 0:
            x = rANTdom_else_none(self.place.bees)
            if x:
                self.eat_bee(x)
...
# The WallAnt class
class WallAnt(Ant):
    """Wall ant does nothing in each turn"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            """Queen itself does not obtain buff."""
            ScubaThrower.action(self, gamestate)

            now = self.place.exit
            while now:
                if now.ant and now.ant not in QueenAnt.buffed:
                    QueenAnt.buffed.append(now.ant)
                    now.ant.damage *= 2
                now = now.exit
        else:
...
def aoe(insect, damage):
    lst = list(insect.place.bees)
    for x in lst:
        x.reduce_armor(damage)
